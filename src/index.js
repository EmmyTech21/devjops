...
function Menu(){
  return <main className='menu'>
     ...
     {devData.map((jobdev) =>(
      <Product jobobj={jobdev} key ={jobdev.name} />))}
...
function Product(props){
  ...
  return( <div className='product'>
    <img  className='logo' src={props.jobobj.logo}alt={props.jobobj.id}/>
    <li className='list'>
      <div>
        <div className='text-1'>
      <p>{props.jobobj.postedAt } <span>.</span> </p>
      <span>{props.jobobj.contract}</span>
      </div>
      <div className='text-2'>
      <h3>{props.jobobj.position}</h3>
      <h6>{props.jobobj.company}</h6>
      <a href='jjjj'>{props.jobobj.location}</a>
      </div>
      </div>
    </li>
  </div>)
